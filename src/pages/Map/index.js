...
export default class Map extends React.Component {
    ...
    createCircle(point, name, count, id, zoom){
        ...
        label.on('click',()=>{
            // console.log(Number(zoom), [parseFloat(point[0]), parseFloat(point[1])])
            // console.log(this.map.setZoomAndCenter)
            // console.log(this.map.options)
            //调用renderOverLays,获取该区域下的房源数据
            this.renderOverLays(id)
            //放大地图，以当前点击的覆盖物为中心放大
            this.map.setZoomAndCenter(zoom, [parseFloat(point[0]), parseFloat(point[1])])
            this.map.options.zoom = zoom
            this.map.options.center = point
            //清除当前覆盖物信息
            this.map.clearMap()
        })
        this.map.add(label);
